...
Source: https://github.com/NUST-Machine-Intelligence-Laboratory/Iris
  file: utils/image_utils.py (upstream)

Adaptation: ``import matplotlib`` is guarded because matplotlib is not a
Scope dependency; it is only used by ``colorize_depth_map``, which is not
on the depth-inference path. All resize helpers used at runtime are
matplotlib-free.
"""

from PIL import Image
try:
    import matplotlib  # only used by colorize_depth_map (not on the inference path)
except ImportError:
    matplotlib = None
import numpy as np
